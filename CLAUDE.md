...
| **cubing_live_api.py** | cubing.com live results — WebSocket feed (`wss://cubing.com/ws`); finished-round results for Chinese comps, days before the WCA upload |
...
11. **cubing.com results come from the live WebSocket, not a REST API:** cubing.com has no results *REST* endpoint — `/results`, `/podiums`, `/live` etc. all 404 (`live` in the detail is just a `1` flag). But the live page (`/live/{alias}`) is a Vue SPA backed by a raw-JSON WebSocket at `wss://cubing.com/ws` that serves structured per-attempt results (centiseconds, WCA Latin names, record tags) **days before** the WCA upload. `cubing_live_api.py` reads this (see its module docstring for the protocol); `records` merges it with the WCA REST scan and **skips WCA-side comps already covered by cubing** so each result alerts once (cubing leads). Only **finished** rounds are read — in-progress rows mutate and dedup keys on the result id. Most Chinese comps are NOT on WCA Live either (in-house scoretaking). The WCA REST `/competitions/{id}/results` remains a fallback and lags several days, so `records` keeps `--ended-days 10` (was 2): a wide re-scan window. Re-scanning is safe — `seen_rest_result_ids` / `seen_cubing_result_ids` dedup, so the only cost is API calls (daily-records.yml `timeout-minutes: 15`). Disable the feed with `--no-cubing-live`.
